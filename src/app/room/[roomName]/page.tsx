...
// Main component for the room page
export default function RoomPage({ params }: { params: { roomName: string } }) {
  ...
  // --- LiveKit Connection and Teardown ---
  const handleEnterRoom = async () => {
    ...
    try {
      ...
      setIsMuted(false); // Ensure UI reflects the initial state
...
function Lobby({ roomName, isConnecting, onEnterRoom }: { roomName: string; isConnecting: boolean; onEnterRoom: () => void; }) {
    ...
    <div className="min-h-screen p-4 md:p-8 flex items-center justify-center">
      <div className="w-full max-w-md">
        <div className="bg-[#080808] p-8 rounded-2xl flex flex-col floating-glow border border-[#222]">
          ...
          <p className="text-center text-gray-400 mb-8">Room: <span className="font-mono">{roomName}</span></p>
          ...
          <div className="mb-8">
            ...
            <div className="flex items-center mt-2 bg-[#111] rounded-xl p-3 border border-[#333]">
              <span className="font-mono text-sm text-white mr-4 truncate">{`.../room/${roomName}`}</span>
...
function InCall({ participants, speakingParticipants, localParticipantSid, isMuted, toggleMute, onLeaveRoom }: any) {
    return (
        <div className="min-h-screen p-4 md:p-8 flex flex-col items-center justify-center">
            <div className="w-full max-w-2xl text-center">
                ...
                <div className="space-y-4">
                    {participants.map((p: Participant) => {
                     ...
                     {participants.length === 0 && <p className="text-gray-500">Connecting to the room...</p>}
                     {participants.length === 1 && <p className="text-gray-500">You're the first one here. Share the link to invite others.</p>}
                </div>
            </div>
            ...
            <div className="fixed bottom-8 flex justify-center w-full">
                <div className="control-bar flex items-center space-x-3 p-2 rounded-full">
                    ...
                    <button className="control-btn p-3 rounded-full">
